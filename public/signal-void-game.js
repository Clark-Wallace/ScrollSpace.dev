class FishTankGame {
    ...
    setupEventListeners() {
        ...
        console.log('Looking for test signal button:', testSignalBtn);
        if (testSignalBtn) {
            console.log('Found test signal button!');
            testSignalBtn.addEventListener('click', () => {
                console.log('Test signal clicked!');
                // Set some test values
                this.score = Math.floor(Math.random() * 1000) + 100;
                this.fishEatenCount = Math.floor(Math.random() * 10) + 1;
                this.timeElapsed = Math.floor(Math.random() * 300) + 30;
                
                // Show the signal modal
                this.showSignalModal();
            });
        } else {
            console.error('Test signal button not found!');
        }
        ...
        window.addEventListener('keydown', (e) => {
            ...
            // Press 'T' to test signal
            if (e.key === 't' || e.key === 'T') {
                console.log('T key pressed - testing signal');
                // Set some test values
                this.score = Math.floor(Math.random() * 1000) + 100;
                this.fishEatenCount = Math.floor(Math.random() * 10) + 1;
                this.timeElapsed = Math.floor(Math.random() * 300) + 30;
    ...
    async fetchSignalMessage() {
        ...
        // Show loading state
        signalTextEl.innerHTML = '<h2 style="font-style: italic;">Decoding signal...</h2>';
        ...
        try {
            ...
            if (response.ok) {
                ...
                // Parse the AI response into title, concept, and optional tagline
                const lines = signal.split('\n').filter(line => line.trim());
                let html = '';
                
                // Look for Title:
                const titleMatch = signal.match(/Title:\s*(.+)/i);
                if (titleMatch) {
                    html += `<h3>${titleMatch[1]}</h3>`;
                }
                
                // Look for Concept:
                const conceptMatch = signal.match(/Concept:\s*([^]*?)(?=Tagline:|$)/i);
                if (conceptMatch) {
                    html += `<p>${conceptMatch[1].trim()}</p>`;
                }
                
                // Look for Tagline:
                const taglineMatch = signal.match(/Tagline:\s*(.+)/i);
                if (taglineMatch) {
                    html += `<p class="tagline">${taglineMatch[1]}</p>`;
                }
                
                // If no structured format, just display as is
                if (!html) {
                    html = `<h3>Emergent Signal</h3><p>${signal}</p>`;
                }
                
                signalTextEl.innerHTML = html;
            } else {
            ...
            signalTextEl.innerHTML = '<h2>The signal has emerged</h2>';
        }
    }
